module.exports.config = {
	...
	version: "2.0.1",
	hasPermssion: 0,
	credits: "CatalizCS", //thêm ảnh: BEARZ 
	description: "3 cây dành cho nhóm có đặt cược",
  ...
  var money = data.money     
	...
	switch (args[0]) {
      ...
      await Currencies.decreaseMoney(event.senderID, Number(args[1]));
			...
			return api.sendMessage(`Bàn 3 cây của bạn đã được tạo thành công. Để tham gia bạn hãy nhập '/3cay join'`, event.threadID, event.messageID);
		}
		...
		case "-j": {
			...
			if (money < values.rateBet) return api.sendMessage(`Bạn không đủ ${values.rateBet}$ để tham gia bàn 3 cây này`,event.threadID,event.messageID)
		...
		case "leave":
		case "-l": {
			if (typeof values.player == "undefined") return api.sendMessage("Hiện tại chưa có bàn 3 cây nào, bạn có thể tạo bằng cách sử dụng 'baicao create'", threadID, messageID);
    ...
    case 'check': {
      for (const key in values.player) {
		...
		default: {
			console.log("[ BEARZ ] » Hi, have a good day.")
		}
	}
}
...
module.exports.onLoad = async () => {
	console.log("====== 3CAY LOADED SUCCESSFULLY ======") };
...
function getLinkCard(Value, Suit) {
  return `https://raw.githubusercontent.com/LittleBearz160/52-poker-card/main/cards/${Value == "J" ? "jack" : Value == "Q" ? "queen" : Value == "K" ? "king" : Value == "A" ? "ace" : Value}_of_${Suit}.png`;
}
