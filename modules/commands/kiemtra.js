...
module.exports.run = async ({ args, api, event, Currencies, client }) => {
   ...
   if (args.length == 0) return api.sendMessage(` ===  Bạn có thể dùng  === \n--------\n=> kiemtra ndfb => Lọc thành viên bị bay acc khỏi nhóm\n--------\n=> kiemtra del => Lọc thành viên khỏi nhóm theo số tin nhắn\n--------\n=> kiemtra onl => Xem thời gian hoạt động bot onl \n--------\n=> kiemtra tt => Check độ tương tác của bạn\n--------\n=> kiemtra box => Lọc nhóm dưới 4 thành viên\n--------\n=> kiemtra all => kiemtra tất cả độ tương tác các thành viên trong box\n--------\n=> kiemtra covid => Xem thông tin covid\n--------\n=> kiemtra mayman => Xem Tỉ lệ % may mắn của bạn?\n--------\n=> kiemtra nude => Check những ảnh gợi cảm\n--------\n    === 「${timeNow}」 ===`, event.threadID, event.messageID);
    var arr = [];
  ...
  return api.sendMessage(
	...
	let seconds = Math.floor(time % 60);
      const timeStart = Date.now();
       return api.sendMessage('=> Đang kiểm tra kết nối vui lòng chờ... !', event.threadID, (err, info) => {
    setTimeout(() => {
      api.sendMessage(`=> Ping: ${(Date.now() - timeStart)}ms \n=> Thời gian bot hoạt động: ${hours}:${minutes}:${seconds}`, event.threadID, event.messageID);
    }, 200);
  }, event.messageID);
