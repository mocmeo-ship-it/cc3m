...
module.exports.run = async function ({ api, event, args, Users, permssion, getText }) {
    ...
    switch (args[0]) {
        ...
        case "add": {
            if (permssion != 2) return api.sendMessage("⚡️ Có Lồn Quyền Mà Dùng Nhóc Ơi ?", threadID, messageID);
        ...
        case "rm":
        case "delete": {
            if (permssion != 2) return api.sendMessage("⚡️ Mày Đéo Có Quyền Đâu Nhóc ?", threadID, messageID);
            if(event.type == "message_reply") { content[0] = event.messageReply.senderID }
        ...
        case 'boxonly': {
            if (permssion != 1) return api.sendMessage("⚡️ Địt Mẹ Mày Có Quyền Đéo Đâu Mà Dùng ?", threadID, messageID);
        const { resolve } = require("path");
  ...
  case '-o': {
            //---> CODE ADMIN ONLY<---//
            if (permssion != 2) return api.sendMessage("⚡️ Thằng Chó Mày Đéo Có Quyền Đâu Đĩ", threadID, messageID);
