...
module.exports.languages = {
  "vi": {
    ...
    "moneyBetNotEnough": "[ LOTO ] Số dư của bạn không đủ, bạn cần %1 đô!",
    "limitBet": "[ LOTO ] Số tiền đặt không được dưới 50$!",
...
module.exports.onLoad = async () => {
  await require('axios').get("https://raw.githubusercontent.com/RFS-ADRENO/mirai-modules/main/version.json").then(res => {
    if (res.data["loto_x024"] != this.config.version) console.log("-LOTO ĐÃ CÓ PHIÊN BẢN MỚI, LIÊN HỆ ĐỂ ĐƯỢC CẬP NHẬT-");
  });
  let path = __dirname + '/loto/';
  if (!fs.existsSync(path)) fs.mkdirSync(path, { recursive: true });
  await require("axios").get("https://raw.githubusercontent.com/LittleBearz160/GbanBearz/main/dataloto.json").then(async (res) => {
